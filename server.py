...
async def _files_post(
    ...
    *,
    ...
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.post(url, json=body, headers=_headers(token))
        r.raise_for_status()
        return r.json()
...
async def upload_asset(
    ...
    with open(file_path, "rb") as fh:
        ...
        async with httpx.AsyncClient(timeout=60) as c:
            r = await c.post(url, files=files, headers=_headers(token))
            r.raise_for_status()
            return r.json()
...
def main() -> None:
    ...
    logger.info(
        "Starting IUDX MCP Server — transport=%s base_url=%s rs_base_url=%s rsp_base_url=%s files_base_url=%s",
        transport, BASE_URL, RS_BASE_URL, RSP_BASE_URL, FILES_BASE_URL,
    )
